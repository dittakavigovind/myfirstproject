...
import { ArrowLeft, Sparkles, Sun, Moon, Hash, Droplet } from "lucide-react";
...
export default function HoroscopeDetail() {
    ...
    useEffect(() => {
        // Set initial active sign to user's moonSign if available
        if (user?.birthDetails?.moonSign) {
    ...
    return (
        <div className="relative min-h-screen pb-6 pt-6 max-w-md mx-auto overflow-hidden shadow-2xl shadow-electric-violet/5">
            ...
            {/* Header */}
            <header className="flex items-center justify-between mb-6 relative z-10 px-4">
                <button
                    onClick={() => router.back()}
                ...
                <h1 className="text-xl font-bold text-white tracking-wide">Daily Reading</h1>
                <div className="w-10" /> {/* Spacer */}
            </header>

            <main className="relative z-10 px-4 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
