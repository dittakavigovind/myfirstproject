...
export default function PanchangPage() {
    ...
    useEffect(() => {
        const initFromUrl = async () => {
            if (params.slug && params.slug.length >= 2) {
                ...
                // Handle DD-MonthName-YYYY (New format: 2 segments)
                ...
                // Handle DD/MM/YYYY (Old format: 4 segments)
                else if (params.slug.length === 4) {
                    newDate = new Date(`${yearParam}-${monthParam}-${dayParam}`);
                }
    ...
    const updateUrl = (newPlace, newDate) => {
        ...
        // Extract only the city name (first part before comma) to keep URL clean
        const cityName = newPlace.split(',')[0].trim();
        const citySlug = encodeURIComponent(cityName);

        router.push(`/panchang/${citySlug}/${d}-${mName}-${y}`, { scroll: false });
    };
                                ...
                                <div className="w-full md:flex-1 space-y-1">
                                    ...
                                    <div className="relative group bg-white/90 hover:bg-white rounded-xl flex items-center h-12 transition-all shadow-lg shadow-black/5">
                                        ...
                                        <DatePicker
                                            ...
                                            className="w-full h-full border-none focus:ring-0 text-slate-800 font-bold pl-10 pr-2 outline-none cursor-pointer bg-transparent rounded-xl text-sm placeholder-slate-400"
                                            showMonthDropdown
                                            showYearDropdown
                                            dropdownMode="select"
