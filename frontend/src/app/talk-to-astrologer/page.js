...
import { Star, CheckCircle, Phone, Search } from 'lucide-react';
...
export default function TalkPage() {
    ...
    useEffect(() => {
        if (featureFlags && featureFlags.enableCall === false) {
            router.push('/');
        }
    }, [featureFlags, router]);
    ...
    const handleCallAction = (e, astro) => {
        e.stopPropagation();
        if (!user) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 pb-20">
            ...
            <HeroSection icon="📞" align="center" extraPaddingBottom={true}>
                ...
                >
                    <span className="inline-block py-1 px-3 rounded-full bg-white/5 border border-white/10 text-orange-300 text-[10px] font-bold tracking-[0.2em] uppercase mb-4 backdrop-blur-md shadow-lg shadow-black/10">
                        ✨ Voice Connect
                    </span>
                    <h1 className="text-3xl md:text-5xl font-black mb-3 leading-tight tracking-tight text-white">
                        Talk to <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 via-amber-200 to-yellow-300">Astrologers</span>
                    </h1>
                    <p className="text-slate-300/90 max-w-xl text-sm md:text-base font-medium leading-relaxed mb-8">
                        Get instant answers via voice call. Connect with certified experts now.
                    </p>
                    ...
                    <div className="w-full max-w-md relative">
                        <input
                            ...
                            className="w-full px-6 py-4 rounded-2xl bg-white/10 border border-white/20 text-white placeholder-slate-400 focus:outline-none focus:border-amber-400/50 focus:bg-white/15 backdrop-blur-md transition-all shadow-xl"
                        />
                                    ...
                                    <div className="flex-1 min-w-0 pt-1">
                                        ...
                                        <div className="space-y-1 text-xs text-slate-500">
                                            ...
                                            <p className="font-bold text-slate-900">
                                                ₹{astro.charges?.callPerMinute || astro.charges?.chatPerMinute || 0}<span className="text-slate-400 font-normal">/min</span>
                                            </p>
                                        </div>
                                    </div>

                                    {/* Right: Action - Call Only */}
                                    <div className="absolute bottom-4 right-4 flex flex-col gap-2">
                                        <button
                                            className="border border-green-500 text-green-600 hover:bg-green-50 font-bold py-1.5 px-4 rounded-lg text-xs transition-all bg-white flex items-center justify-center gap-2 hover:scale-105 min-w-[90px]"
                                            onClick={(e) => handleCallAction(e, astro)}
                                        >
                                            <Phone size={14} /> Call
                                        </button>
                                    </div>
