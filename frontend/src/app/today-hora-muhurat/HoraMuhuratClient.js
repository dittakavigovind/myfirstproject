...
export default function HoraMuhuratClient() {
    return (
        <Suspense fallback={
...
function HoraContent() {
    ...
    useEffect(() => {
        ...
        document.title = `${t('todayHoraMuhurat', lang)} - ${dateStr} - ${cityName} | Way2Astro`;
        
        const newUrl = `/today-hora-muhurat?city=${encodeURIComponent(cityName)}&date=${date.toISOString().split('T')[0]}`;
    ...
    const fetchHora = async () => {
        ...
        try {
            ...
            console.error('Fetch Hora Error:', err);
        } finally {
            setLoading(false);
        }
    };
