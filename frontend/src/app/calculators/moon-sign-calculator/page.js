...
function MoonSignCalculatorContent() {
    ...
    useEffect(() => {
        if (isInitialized && searchParams.get('action') === 'calculate' && formData.date && formData.lat && !result && !loading) {
            calculateMoonSign({ preventDefault: () => { } });
        }
            ...
            {!result ? (
                <HeroSection icon="🌙" align="center" extraPaddingBottom={true}>
                    ...
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center">
                        ...
                        <h1 className="text-4xl md:text-5xl font-black mb-4 text-white">Moon Sign <span className="text-indigo-400">Calculator</span></h1>
