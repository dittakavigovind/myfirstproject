...
export default function HomeClient() {
  ...
  useEffect(() => {
    if (featureFlags?.heroSection?.showCarousel && featureFlags.heroSection.carouselImages?.length > 1) {
      const interval = setInterval(() => {
        setCurrentImageIndex(prev => (prev + 1) % featureFlags.heroSection.carouselImages.length);
      }, 5000);
      return () => clearInterval(interval);
    }
  }, [featureFlags?.heroSection]);
                  ...
                  <div className="flex flex-col md:flex-row items-center justify-between gap-12 md:gap-16 text-center md:text-left">
                    ...
                    {((featureFlags?.enableChat || featureFlags?.enableCall) || (!featureFlags?.heroSection?.showCarousel && (featureFlags?.promotionImage || featureFlags?.promotionVideoUrl))) && (
                      <div className="flex-1 w-full md:max-w-md">
                        <div className="bg-white/5 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl relative overflow-hidden group max-w-[400px] mx-auto aspect-video md:aspect-auto xl:aspect-video lg:aspect-auto md:max-w-none flex flex-col justify-center">
