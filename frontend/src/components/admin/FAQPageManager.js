...
export default function FAQPageManager() {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const res = await API.put(`/page-content/${selectedPage}`, { faqs });
            if (res.data.success) {
        ...
        <div className="space-y-8">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-100">
                <div>
                    ...
                    <p className="text-sm text-slate-500">Manage frequently asked questions for specific pages</p>
                </div>
                <button
                    onClick={handleSave}
                    ...
                    {loading ? (
                        ...
                        <div className="mb-8">
                            <h2 className="text-lg font-bold text-slate-800 mb-6 border-b pb-2">
                                Editing FAQs for: <span className="text-blue-600">{pages.find(p => p.id === selectedPage)?.name || selectedPage}</span>
                            </h2>
