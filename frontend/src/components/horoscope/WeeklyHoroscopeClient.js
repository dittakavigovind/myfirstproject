...
import { Droplet, Calendar as CalendarIcon, Sparkles, Zap, Compass } from 'lucide-react';
...
export default function WeeklyHoroscopeClient() {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                const res = await API.get('/page-content/weekly-horoscope');
                ...
                console.error("Failed to fetch weekly horoscope page content:", err);
            }
        };
        fetchContent();
    }, []);
        ...
        <div className="min-h-screen bg-slate-50 pb-20 overflow-x-hidden">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-12 w-full">
                    ...
                    >
                        ...
                        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl bg-white/20 backdrop-blur-md border border-white/20 text-white font-bold mb-4 shadow-lg">
                            ...
                            <span>{format(weekStart, 'MMM dd')} - {format(weekEnd, 'MMM dd, yyyy')}</span>
                        </div>
                        <p className="text-white/80 font-medium max-w-lg mx-auto md:mx-0 text-sm md:text-base leading-relaxed">
                            Discover what the stars have planned for your week. Navigate your days with celestial insight.
                        </p>
                    </motion.div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-20 -mt-16">
                ...
                <motion.div
                    ...
                    className="bg-white rounded-[2.5rem] shadow-2xl border border-slate-100 p-2 md:p-2 mb-12 flex flex-col items-center gap-8 mx-auto"
                >
                    {/* Zodiac Selector - Squarcle Grid */}
                ...
                <AnimatePresence mode="wait">
                    {loading ? (
                        ...
                        <motion.div
                            key={selectedSign + weekStart.toString()}
                            ...
                            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl border border-slate-100 relative overflow-hidden group">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 relative z-10">
                                    <div className="flex items-center gap-5">
                                        ...
                                        <div>
                                            ...
                                            <p className={`text-sm font-bold uppercase tracking-widest ${theme.text} opacity-80`}>{horoscopeData.title || 'Weekly Forecast'}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 relative z-10">
                                    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100/60 flex flex-col items-center justify-center text-center shadow-sm hover:shadow-md transition-shadow group/stat">
                                        ...
                                        <span className="text-[10px] uppercase font-bold text-slate-400 mb-1 tracking-widest">Luck</span>
                                        <span className="text-3xl font-black text-slate-900 capitalize">{horoscopeData.luck || 'N/A'}</span>
                                    </div>
                                    ...
                                    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100/60 flex flex-col items-center justify-center text-center shadow-sm hover:shadow-md transition-shadow group/stat">
                                        ...
                                        <span className="text-[10px] uppercase font-bold text-slate-400 mb-1 tracking-widest">Energy</span>
                                        <span className="text-3xl font-black text-slate-900 uppercase">{horoscopeData.energy || 'N/A'}</span>
                                    </div>

                                    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100/60 flex flex-col items-center justify-center text-center shadow-sm hover:shadow-md transition-shadow group/stat md:col-span-2 lg:col-span-1">
                                        <div className={`w-12 h-12 mb-3 rounded-full ${theme.bg} flex items-center justify-center group-hover/stat:scale-110 transition-transform`}>
                                            <Compass className={`w-5 h-5 ${theme.text}`} />
                                        </div>
                                        <span className="text-[10px] uppercase font-bold text-slate-400 mb-1 tracking-widest">Cosmic Advice</span>
                                        <p className="text-sm font-bold text-slate-800 text-center leading-tight line-clamp-3">
                                            {horoscopeData.advice || "Stay positive and aligned with your goals."}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-[2.5rem] p-12 text-center shadow-sm min-h-[300px] flex flex-col items-center justify-center border border-slate-100">
                            ...
                            <p className="text-slate-500 max-w-sm mx-auto font-medium leading-relaxed">
                                The forecast for <span className="font-bold text-slate-800 capitalize decoration-2 underline decoration-slate-200">{selectedSign}</span> on week <span className="font-bold text-slate-700">{format(weekStart, 'MMM dd')}</span> hasn't been written yet.
                            </p>
                        </div>
                    )}
                </AnimatePresence>
