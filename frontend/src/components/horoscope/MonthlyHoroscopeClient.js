...
import { Droplet, Calendar as CalendarIcon, Briefcase, Heart, Activity } from 'lucide-react';
...
export default function MonthlyHoroscopeClient() {
    ...
    useEffect(() => {
        const fetchContent = async () => {
            try {
                const res = await API.get('/page-content/monthly-horoscope');
                ...
                console.error("Failed to fetch monthly horoscope page content:", err);
            }
        };
        fetchContent();
    }, []);
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-12 w-full">
                    ...
                    >
                        ...
                        <p className="text-white/80 font-medium max-w-lg mx-auto md:mx-0 text-sm md:text-base leading-relaxed">
                            Prepare for what's coming. Detailed forecasts for career, love, and health.
                        </p>
                    </motion.div>
                ...
                <AnimatePresence mode="wait">
                    {loading ? (
                        <div key="loader" className="bg-white rounded-[2.5rem] p-16 text-center shadow-lg border border-slate-100 min-h-[400px] flex flex-col items-center justify-center">
                            ...
                            <p className="text-slate-400 font-bold text-lg animate-pulse tracking-wide">Reading the stars...</p>
                        </div>
                    ) : horoscopeData && horoscopeData.overview ? (
                        <motion.div
                            key={selectedSign + currentDate.toString()}
                            ...
                            <div className="bg-white rounded-[2.5rem] p-8 md:p-12 shadow-2xl border border-slate-100 relative overflow-hidden group">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 relative z-10">
                                    <div className="flex items-center gap-5">
                                        ...
                                        <div>
                                            <h2 className="text-3xl md:text-4xl font-black text-slate-800 capitalize leading-none mb-2">{selectedSign} Monthly</h2>
                                            <p className={`text-sm font-bold uppercase tracking-widest ${theme.text} opacity-80`}>{horoscopeData.title || 'Monthly Forecast'}</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                                    <div className={`bg-slate-50 rounded-2xl p-6 border border-slate-100/60 transition-all hover:bg-white hover:shadow-lg group/cat`}>
                                        <div className={`w-12 h-12 mb-4 rounded-2xl ${theme.bg} flex items-center justify-center group-hover/cat:scale-110 transition-transform`}>
                                            <Briefcase className={`w-6 h-6 ${theme.text}`} />
                                        </div>
                                        <h3 className="text-lg font-black text-slate-800 mb-2">Career</h3>
                                        <p className="text-sm text-slate-500 leading-relaxed font-medium">
                                            {horoscopeData.career || "Focus on your goals and stay consistent."}
                                        </p>
                                    </div>
                                    ...
                                    <div className={`bg-slate-50 rounded-2xl p-6 border border-slate-100/60 transition-all hover:bg-white hover:shadow-lg group/cat`}>
                                        ...
                                        <h3 className="text-lg font-black text-slate-800 mb-2">Love</h3>
                                        <p className="text-sm text-slate-500 leading-relaxed font-medium">
                                            {horoscopeData.love || "Open your heart to new possibilities."}
                                        </p>
                                    </div>
                                    ...
                                    <div className={`bg-slate-50 rounded-2xl p-6 border border-slate-100/60 transition-all hover:bg-white hover:shadow-lg group/cat`}>
                                        ...
                                        <h3 className="text-lg font-black text-slate-800 mb-2">Health</h3>
                                        <p className="text-sm text-slate-500 leading-relaxed font-medium">
                                            {horoscopeData.health || "Prioritize your well-being and rest."}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-[2.5rem] p-12 text-center shadow-sm min-h-[300px] flex flex-col items-center justify-center border border-slate-100">
                            ...
                            <p className="text-slate-500 max-w-sm mx-auto font-medium leading-relaxed">
                                The forecast for <span className="font-bold text-slate-800 capitalize decoration-2 underline decoration-slate-200">{selectedSign}</span> for <span className="font-bold text-slate-700">{format(currentDate, 'MMMM yyyy')}</span> hasn't been written yet.
                            </p>
                        </div>
                    )}
                </AnimatePresence>
